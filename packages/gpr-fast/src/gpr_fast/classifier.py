...
def evaluate_encoded_ruleset(
    complemented_samples,
    rule_columns,
    rule_lengths,
    threshold
):
    n_samples = complemented_samples.shape[0]
    n_rules = rule_lengths.shape[0]

    predictions = np.empty(n_samples, dtype=np.int32)

    for sample_index in prange(n_samples):
        score = 0.0

        for rule_index in range(n_rules):
            activation = 1.0

            for literal_index in range(rule_lengths[rule_index]):
                column_index = rule_columns[rule_index, literal_index]
                activation *= complemented_samples[sample_index, column_index]

            score += activation

        predictions[sample_index] = score > threshold
...
class GPR_FAST:
    ...
    def _init_generate_population_function(self, primitive_set: gep.PrimitiveSet, max_n_of_rules: int, max_n_of_ands: int):
    ...
    def _init_evaluation_function(self, toolbox):
        def evaluate(individual):
            rule_columns, rule_lengths = self._chromosome_to_rule_columns(individual)
            predictions = evaluate_encoded_ruleset(self.complemented_samples, rule_columns, rule_lengths, self.threshold)
    ...
    def _chromosome_to_rule_columns(self, chromosome):
        ...
        for gene_index, gene in enumerate(chromosome):
            ...
            for symbol in gene.kexpression:
                if symbol.arity == 0:
                    column_indices.append(self.terminal_to_column[symbol.name])
                    
            if not column_indices:
                raise ValueError(
                    f"Gene {gene_index} does not contain any active terminals. "
                    f"K-expression: {gene.kexpression}"
                )
                
            encoded_rules.append(column_indices)
            
        if not encoded_rules:
            raise ValueError(
                "The chromosome does not contain any genes."
            )
    
        longest_rule = max(map(len, encoded_rules))
        
        n_rules = len(encoded_rules)
    
        rule_columns = np.full(
            shape=(n_rules, longest_rule),
            fill_value=-1,
            dtype=np.int64,
        )
    
        rule_lengths = np.zeros(
            n_rules,
            dtype=np.int64,
        )
    
        for rule_index, column_indices in enumerate(encoded_rules):
            rule_length = len(column_indices)
    
            rule_columns[rule_index, :rule_length] = column_indices
    
            rule_lengths[rule_index] = rule_length
    
        return rule_columns, rule_lengths
    ...
    def fit(self, x: np.ndarray, y: np.ndarray):
        ...
        self.terminal_to_column = dict(
            zip(
                self.primitive_set.input_names, 
                range(self.complemented_samples.shape[1])
            )
        )
    ...
    def predict(self, x: np.ndarray) -> np.ndarray:
        ...
        rule_columns, rule_lengths = self._chromosome_to_rule_columns(self._best_fit)
        return evaluate_encoded_ruleset(complemented_samples, rule_columns, rule_lengths, self.threshold)